...
void main() {
  ...
  group('Pathway Links Data Tests', () {
    test(
      'allTrainingPathways includes enriched mobile and web pathways',
      () async {
        ...
        expect(mobilePathway.links.length, greaterThanOrEqualTo(1));

        final androidPathway = pathways.firstWhere(
          (p) => p.name == 'Android Development',
        );
        expect(androidPathway.links.length, greaterThanOrEqualTo(1));

        final iosPathway = pathways.firstWhere(
          (p) => p.name == 'iOS Development',
        );
        expect(iosPathway.links.length, greaterThanOrEqualTo(1));

        final fullStackPathway = pathways.firstWhere(
          (p) => p.name == 'Full-Stack & General Development',
        );
        expect(fullStackPathway.links.length, greaterThanOrEqualTo(1));

        final frontEndPathway = pathways.firstWhere(
          (p) => p.name == 'JavaScript & Front-End',
        );
        expect(frontEndPathway.links.length, greaterThanOrEqualTo(1));

        final backEndPathway = pathways.firstWhere(
          (p) => p.name == 'Back-End Development',
        );
        expect(backEndPathway.links.length, greaterThanOrEqualTo(1));

        final generalWebPathway = pathways.firstWhere(
          (p) => p.name == 'General Web / CMS',
        );
        expect(generalWebPathway.links.length, greaterThanOrEqualTo(1));

        final typeScriptPathway = pathways.firstWhere(
          (p) => p.name == 'TypeScript Development',
        );
        expect(typeScriptPathway.links.length, greaterThanOrEqualTo(1));
        // Only free links are kept (no paid exams or subscriptions), and every
        // pathway still has at least one.
        for (final p in pathways) {
          expect(p.links, isNotEmpty, reason: p.name);
          expect(p.links.where((l) => l.cost == TrainingCost.paid), isEmpty);
        }
      },
    );
    ...
    test(
      'pathways offer extensive free certification paths across tech domains',
      () async {
        final pathways = await allTrainingPathways();
        ...
        final pathwaysWithFree = pathways
            .where((p) => p.links.any((l) => l.cost != TrainingCost.paid))
            .toList();
        expect(pathwaysWithFree.length, equals(pathways.length));

        // Check specific domains have recognized free certifications
        final mobile = pathways.firstWhere(
          (p) => p.name == 'Mobile App Development (Cross-Platform)',
        );
        expect(
          mobile.links.any((l) => l.label.contains('CS50M') && l.isFree),
          isTrue,
        );

        final fullStack = pathways.firstWhere(
          (p) => p.name == 'Full-Stack & General Development',
        );
        expect(
          fullStack.links.any((l) => l.label.contains('CS50W') && l.isFree),
          isTrue,
        );
        expect(
          fullStack.links.any(
            (l) => l.label.contains('freeCodeCamp') && l.isFree,
          ),
          isTrue,
        );

        final cyber = pathways.firstWhere(
          (p) => p.name == 'Cybersecurity Analysis',
        );
        expect(
          cyber.links.any((l) => l.label.contains('ISC2') && l.isFree),
          isTrue,
        );

        final csharp = pathways.firstWhere(
          (p) => p.name == '.NET / C# Development',
        );
        expect(
          csharp.links.any((l) => l.label.contains('Microsoft') && l.isFree),
          isTrue,
        );
        ...
        final gameDev = pathways.firstWhere(
          (p) => p.name == 'Game Development',
        );
        expect(gameDev.field, equals('Game Dev'));
        expect(
          gameDev.links.any((l) => l.isFree && l.provider == 'Harvard'),
          isTrue,
        );

        final itSupport = pathways.firstWhere(
          (p) => p.name == 'IT Support & Help Desk',
        );
        expect(itSupport.field, equals('IT & Support'));
        expect(itSupport.links.any((l) => l.isFree), isTrue);

        final dataEng = pathways.firstWhere(
          (p) => p.name == 'Data Engineering & Big Data',
        );
        expect(dataEng.field, equals('Data & AI'));
        expect(dataEng.links.any((l) => l.isFree), isTrue);

        final management = pathways.firstWhere(
          (p) => p.name == 'Project, Program & Product Management',
        );
    ...
    test(
      'metadata fields (field, provider, type, isFree) are correctly loaded',
      () async {
        final pathways = await allTrainingPathways();
        for (final p in pathways) {
    ...
    testWidgets('PathwayPage renders without errors', (tester) async {
      ...
      await tester.pumpWidget(const MaterialApp(home: PathwayPage()));
    ...
    testWidgets(
      'PathwayPage searches and filters TypeScript pathways correctly',
      (tester) async {
        SharedPreferences.setMockInitialValues({});
        await tester.pumpWidget(
          const MaterialApp(home: PathwayPage(initialQuery: 'TypeScript')),
        );
        await tester.runAsync(() async {
          await Future.delayed(const Duration(milliseconds: 200));
        });
    ...
    testWidgets('PathwayPage resolves TS abbreviation via alias search', (
      tester,
    ) async {
      SharedPreferences.setMockInitialValues({});
      await tester.pumpWidget(
        const MaterialApp(home: PathwayPage(initialQuery: 'TS')),
      );
      await tester.runAsync(() async {
        await Future.delayed(const Duration(milliseconds: 200));
      });
